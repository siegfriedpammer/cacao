...
\author{
  \authN{Bräutigam Klaus}\\
  \matNr{0825965}\\
  \authI{Institute of Computer Languages}\\
  \authU{Technical University of Vienna}\\
  \email{e0825965@student.tuwien.ac.at}\\\\
  \authN{Bachelor Thesis BSc. in Informatics - Software and Information
  Engineering 033 534 }\\
  \authN{supervised by Univ.Prof. Dr. Andreas Krall}
}
...
far out of the documents scope so we will only touch on the general ideas.
	...
	Before designing new optmization techniques for ABCs it's important to know
	...
	be integreated automatically without developers knowledge or should
	...
	really worth while studying and researching the topic? This is in many cases a difficult
	...
	design and architecture and on the application itself.
		...
		processor systems that didn't really considered modern programming techniques
		...
		applications. For example Java is mainly used due to it's simple and clear
		...
		\subsubsection{Necessarity for ABC Optimization}
		...
		be the architects and implementors target to evolve the language to the most
	...
	dramatically influences the effectiveness of the optimization.
		...
		On the other side Java declares it's integer type explicitly signed and
		...
		interesting source for the beginning because it focuses on especially on 
		...
		and in the past several forms based on Static Single Assignment (SSA) where
		...
		infered from high level code and intermediate representations through
	...
	But not only the array access instructions itself are important for ABCs but
	...
	three deal with the code structure where these ABCs occure. A good explanation
	...
	\cite{OptimizingArrayBoundChecksUsingFlowAnalysis:1993}.
		...
		fractions of execution time. Sometimes Local and Global ABCs are found in
...
should be possible to quickly understand the raw idea described.
	...
	In the authors opinion William H. Harrison was the
	...
	also necessary for more elaborte analyzing of Global ABCs. For really
	...
	constraints to the eSSA instructions and build up an inequality graph IG
	...
	analyze but can in some circiumstances really allow good optimizations. \par
	...
	whole problem. The first idea is based on Guptas approach. Insert ABCs and
	...
	explanation about this is widely out of this documents scope. \par
	...
	array acessing and can lead to deoptimization (OSR at safepoint back to stack)
	if the new conditions can't prove the previously optimizations. But several
	ABCs can be grouped if proveable which leads to further optimizations. All
 	...
 	embedded in the VM and very effective goind by the test results. \par
...
\section{ABCs in Cacao Java Virtual Machine}
	...
	The work on the Cacao Java VM was started in 1996 by Rainhard Grafl and
	...
	\cite{HandbookForJavaCacaoVirtualMachine:1996} in chapter 5.7 Loop Optimization.
		...
		This further constraints possibilities because no global ABCs can be removed. 
		...
		calculating the domintators of the blocks using the Lengauer-Tarjan algorithm 
		...
		After this happend identical loops are merged, nested loops resolved and all together they are topologically sorted.
...
\end{multicols}
		...
		\begin{description}
			...
			\item[BasicBlocks (bb)] is a container that contains code of one
				...
				contained. So this means that a basic block is garanteed to
		...
		Postconditions are that the methods jitdata structure holds correct, executable
		...
		From this data with polynomic behavior a fixed per centage of speed acceleration should
...
\begin{multicols}{2}
		...
		part of the ABC optimization algorithm includes the necessarity for loop detection at the beginning. 
		...
		that normaly do not occur in most other programs. The authors claim that their algorithm is for
