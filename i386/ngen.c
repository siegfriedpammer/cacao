/* i386/ngen.c *****************************************************************
	...
	Last Change: $Id: ngen.c 203 2003-01-23 22:49:45Z twisti $
...
/* 		{p--; M_DST (savfltregs[r], REG_SP, 8 * p);} */
	...
	for (r = savintregcnt - 1; r >= maxsavintreguse; r--) {
 		p--; i386_mov_reg_membase(savintregs[r], REG_SP, p * 4);
	}
	for (r = savfltregcnt - 1; r >= maxsavfltreguse; r--) {
		p--; M_DST (savfltregs[r], REG_SP, 8 * p);
	}
...
#define ICONST(reg,imm) \
   do { \
       i386_mov_imm_reg((imm),(reg)); \
   } while (0) 
...
#define LCONST(reg,imm)	\
    do { \
        i386_mov_imm_reg(((imm) & 0xffffffff),I386_EAX); \
        i386_mov_imm_reg(((imm) >> 32),I386_EDX); \
    } while (0)
		...
		case ICMD_LCONST:     /* ...  ==> ..., constant                       */
			...
			if (iptr->dst->flags & INMEMORY) {
				...
				/*i386_mov_imm_reg(iptr->val.i, d);*/
				printf("No 64bit register available!");
			}
			break;
...
/*  			break;	 */
			d = reg_of_var(iptr->dst, REG_FTMP1);
			a = dseg_addfloat(iptr->val.f);
			printf("FCONST: f=%f x=%x a=%x m=%x\n", iptr->val.f, *((s4 *) (dsegtop + a)), dsegtop, mcodeptr);
			i386_flds_mem(dsegtop + a);
			break;
...
/* 			break; */
			...
			if ((iptr->dst->varkind == LOCALVAR) &&
			    (iptr->dst->varnum == iptr->op1)) {
				printf("ILOAD LOCALVAR: inmemory=%d\n", iptr->dst->flags & INMEMORY);
				break;
			}
			var = &(locals[iptr->op1][iptr->opc - ICMD_ILOAD]);
			printf("ILOAD: %d\n", var->flags & INMEMORY);
...
/*  			break; */
			...
			if ((iptr->dst->varkind == LOCALVAR) &&
			    (iptr->dst->varnum == iptr->op1)) {
				printf("LLOAD LOCALVAR: inmemory=%d\n", iptr->dst->flags & INMEMORY);
				break;
			}
			var = &(locals[iptr->op1][iptr->opc - ICMD_ILOAD]);
			printf("LLOAD: %d\n", var->flags & INMEMORY);
...
/*  			break; */
			if ((src->varkind == LOCALVAR) &&
			    (src->varnum == iptr->op1)) {
				printf("ISTORE LOCALVAR\n");
				break;
			}
			var = &(locals[iptr->op1][iptr->opc - ICMD_ISTORE]);
			if (var->flags & INMEMORY) {
				printf("ISTORE: INMEMORY\n");
				var_to_reg_int(s1, src, REG_ITMP1);
...
/*  			break; */
			if ((src->varkind == LOCALVAR) &&
			    (src->varnum == iptr->op1)) {
				printf("LSTORE LOCALVAR\n");
				break;
			}
			var = &(locals[iptr->op1][iptr->opc - ICMD_ISTORE]);
			if (var->flags & INMEMORY) {
				printf("LSTORE: INMEMORY\n");
				var_to_reg_int(s1, src, REG_ITMP1);
				i386_mov_reg_membase(s1, REG_SP, var->regoff * 8);
...
#define M_COPY(from,to) \
			...
			if ((from->regoff != to->regoff) || \
				...
				if (IS_FLT_DBL_TYPE(from->type)) { \
					...
					M_INTMOVE(from->regoff,to->regoff); \
				}\
			}
		...
		case ICMD_INT2CHAR:   /* ..., value  ==> ..., value                   */
			...
			if (iptr->dst->flags & INMEMORY) {
				if (src->flags & INMEMORY) {
					i386_mov_membase_reg(REG_SP, src->regoff * 8, REG_ITMP1);
					i386_alu_imm_reg(I386_AND, 0x000000ff, REG_ITMP1);
					i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);

				} else {
					i386_mov_reg_membase(src->regoff, REG_SP, iptr->dst->regoff * 8);
					i386_alu_imm_membase(I386_AND, 0x000000ff, REG_SP, iptr->dst->regoff * 8);
				}

			} else {
				if (src->flags & INMEMORY) {
					i386_mov_membase_reg(REG_SP, src->regoff * 8, iptr->dst->regoff);
					i386_alu_imm_reg(I386_AND, 0x000000ff, iptr->dst->regoff);
		...
		case ICMD_LMUL:       /* ..., val1, val2  ==> ..., val1 * val2        */
			...
			if (iptr->dst->flags & INMEMORY) {
				if ((src->flags & INMEMORY) && (src->prev->flags & INMEMORY)) {
					...
					i386_mul_membase(REG_SP, src->prev->regoff * 8);
					i386_mov_membase_reg(REG_SP, src->prev->regoff * 8, REG_ITMP1);
					i386_mov_reg_reg(REG_ITMP2, REG_ITMP2);
					i386_imul_membase_reg(REG_SP, src->regoff * 8 + 4, REG_ITMP2);
					i386_mov_reg_reg(I386_EDX, REG_ITMP1);
					i386_alu_reg_reg(I386_ADD, REG_ITMP2, REG_ITMP1);
					i386_mov_membase_reg(REG_SP, src->regoff * 8, REG_ITMP2);
					i386_imul_membase_reg(REG_SP, src->prev->regoff * 8 + 4, REG_ITMP2);

					i386_alu_reg_reg(I386_ADD, REG_ITMP2, REG_ITMP1);
					i386_mov_reg_reg(REG_ITMP1, I386_EDX);
					i386_mov_reg_membase(I386_EAX, REG_SP, iptr->dst->regoff * 8);
					i386_mov_reg_membase(I386_EDX, REG_SP, iptr->dst->regoff * 8 + 4);
					/* optimized */
/*  					i386_alu_reg_reg(I386_ADD, REG_ITMP2, REG_ITMP1); */
/*  					i386_mov_reg_membase(I386_EAX, REG_SP, iptr->dst->regoff * 4); */
/*  					i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 4 + 4); */
				}
			}
			break;
			...
			if (iptr->dst->flags & INMEMORY) {
				if ((src->flags & INMEMORY) && (src->prev->flags & INMEMORY)) {
					if (src->regoff == iptr->dst->regoff) {
						...
						i386_alu_reg_membase(I386_AND, REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
						...
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->prev->regoff * 8 + 4, REG_ITMP1);
						i386_alu_membase_reg(I386_AND, REG_SP, src->regoff * 8 + 4, REG_ITMP1);
			...
			if (iptr->dst->flags & INMEMORY) {
				if (src->flags & INMEMORY) {
					if (src->regoff == iptr->dst->regoff) {
						...
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->regoff * 8 + 4, REG_ITMP1);
						i386_alu_imm_reg(I386_AND, iptr->val.l >> 32, REG_ITMP1);
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
					}
				}
			}
			...
			if (iptr->dst->flags & INMEMORY) {
				if ((src->flags & INMEMORY) && (src->prev->flags & INMEMORY)) {
					if (src->regoff == iptr->dst->regoff) {
						...
						i386_alu_reg_membase(I386_OR, REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->prev->regoff * 8 + 4, REG_ITMP1);
						i386_alu_reg_membase(I386_OR, REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
			...
			if (iptr->dst->flags & INMEMORY) {
				if (src->flags & INMEMORY) {
					if (src->regoff == iptr->dst->regoff) {
						...
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->regoff * 8 + 4, REG_ITMP1);
						i386_alu_imm_reg(I386_OR, iptr->val.l >> 32, REG_ITMP1);
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
					}
				}
			}
			...
			if (iptr->dst->flags & INMEMORY) {
				if ((src->flags & INMEMORY) && (src->prev->flags & INMEMORY)) {
					if (src->regoff == iptr->dst->regoff) {
						...
						i386_alu_reg_membase(I386_XOR, REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->prev->regoff * 8 + 4, REG_ITMP1);
						i386_alu_reg_membase(I386_XOR, REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
						...
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->prev->regoff * 8 + 4, REG_ITMP1);
						i386_alu_membase_reg(I386_XOR, REG_SP, src->regoff * 8 + 4, REG_ITMP1);
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
					}
				}
			}
			...
			if (iptr->dst->flags & INMEMORY) {
				if (src->flags & INMEMORY) {
					if (src->regoff == iptr->dst->regoff) {
						...
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_membase_reg(REG_SP, src->regoff * 8 + 4, REG_ITMP1);
						i386_alu_imm_reg(I386_XOR, iptr->val.l >> 32, REG_ITMP1);
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8 + 4);
					}
				}
			}
...
/*  			break; */
		...
		case ICMD_L2D:       /* ..., value  ==> ..., (double) value           */
			...
			if (src->flags & INMEMORY) {
				...
				printf("Longs have to be in memory!\n");
			}
			break;
		...
		case ICMD_D2I:
			...
			if (iptr->dst->flags & INMEMORY) {
				...
				a = dseg_adds4(0);
				i386_fistpl_mem(a);
				i386_mov_mem_reg(a, iptr->dst->regoff);
			}
			break;
...
/*  			break; */
			...
			i386_fucompp();
			i386_fnstsw();
			i386_test_imm_reg(0x45, I386_EAX);
...
/*  			break; */
			...
			if (iptr->dst->flags & INMEMORY) {
				i386_mov_membase_reg(s1, OFFSET(java_arrayheader, size), REG_ITMP2);
				i386_mov_reg_membase(REG_ITMP2, REG_SP, iptr->dst->regoff * 8);

			} else {
				i386_mov_membase_reg(s1, OFFSET(java_arrayheader, size), iptr->dst->regoff);
			}
			break;
...
/*  			break; */
			...
			i386_mov_memindex_reg(OFFSET(java_intarray, data[0]), s1, s2, 2, REG_ITMP3);
			if (iptr->dst->flags & INMEMORY) {
...
/*  			break; */
			...
			if (iptr->dst->flags & INMEMORY) {
				i386_mov_memindex_reg(OFFSET(java_intarray, data[0]), s1, s2, 2, REG_ITMP3);
				i386_mov_reg_membase(REG_ITMP3, REG_SP, iptr->dst->regoff * 8);
				i386_mov_memindex_reg(OFFSET(java_intarray, data[0]) + 4, s1, s2, 2, REG_ITMP3);
...
/*  			break; */
			...
			if (src->flags & INMEMORY) {
				...
				i386_mov_reg_memindex(REG_ITMP3, OFFSET(java_objectarray, data[0]), s1, s2, 2);
				i386_mov_membase_reg(REG_SP, src->regoff * 8 + 4, REG_ITMP3);
				i386_mov_reg_memindex(REG_ITMP3, OFFSET(java_objectarray, data[0]) + 4, s1, s2, 2);
			}
			break;
...
/*  			break; */
			...
			var_to_reg_int(s3, src, REG_ITMP3);
			i386_mov_reg_memindex(s3, OFFSET(java_intarray, data[0]), s1, s2, 4);
			break;
...
/*  			break; */
			...
			switch (iptr->op1) {
				...
				case TYPE_ADR:
					if (src->flags & INMEMORY) {
						i386_mov_membase_reg(REG_SP, src->regoff * 8, REG_ITMP1);
						i386_mov_imm_reg(dsegtop, REG_ITMP2);
						i386_mov_reg_membase(REG_ITMP1, REG_ITMP2, a);
					} else {
						i386_mov_imm_reg(dsegtop, REG_ITMP2);
						i386_mov_reg_membase(src->regoff, REG_ITMP2, a);
					}
					break;
				case TYPE_LNG:
					var_to_reg_int(s2, src, REG_ITMP2);
...
/*  			break; */
			...
			switch (iptr->op1) {
				...
				case TYPE_ADR:
					...
					if (iptr->dst->flags & INMEMORY) {
						i386_mov_imm_reg(dsegtop, REG_ITMP2);
						i386_mov_membase_reg(REG_ITMP2, a, REG_ITMP1);
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
					} else {
						i386_mov_imm_reg(dsegtop, REG_ITMP2);
						i386_mov_membase_reg(REG_ITMP2, a, iptr->dst->regoff);
					}
					break;
				case TYPE_LNG:
					d = reg_of_var(iptr->dst, REG_ITMP3);
					if (iptr->dst->flags & INMEMORY) {
						i386_mov_mem_reg(a, REG_ITMP1);
						i386_mov_mem_reg(a, REG_ITMP2);
						i386_mov_reg_membase(REG_ITMP1, REG_SP, iptr->dst->regoff * 8);
						i386_mov_reg_membase(REG_ITMP2, REG_SP, iptr->dst->regoff * 8 + 4);
					} else {
						i386_mov_mem_reg(a, iptr->dst->regoff);
...
#define ALIGNCODENOP do {} while (0)
		...
		case ICMD_IF_LLT:       /* ..., value ==> ...                         */
			...
			/* same as ICMD_IF_LCMPLT with imm->reg move -- TODO: optimize */
			if (src->flags & INMEMORY) {
				int offset;
				i386_mov_imm_reg(iptr->val.l >> 32, REG_ITMP1);
		...
		case ICMD_IF_LLE:       /* ..., value ==> ...                         */
			...
			/* same as ICMD_IF_LCMPLE with imm->reg move -- TODO: optimize */
			if (src->flags & INMEMORY) {
				int offset;
				i386_mov_imm_reg(iptr->val.l >> 32, REG_ITMP1);
				i386_alu_membase_reg(I386_CMP, REG_SP, src->regoff * 8 + 4, REG_ITMP1);
		...
		case ICMD_IF_LGT:       /* ..., value ==> ...                         */
			...
			/* same as ICMD_IF_LCMPGT with imm->reg move -- TODO: optimize */
			if (src->flags & INMEMORY) {
				int offset;
				i386_mov_imm_reg(iptr->val.l >> 32, REG_ITMP1);
				i386_alu_membase_reg(I386_CMP, REG_SP, src->regoff * 8 + 4, REG_ITMP1);
		...
		case ICMD_IF_LGE:       /* ..., value ==> ...                         */
			...
			/* same as ICMD_IF_LCMPGE with imm->reg move -- TODO: optimize */
			if (src->flags & INMEMORY) {
				int offset;
				i386_mov_imm_reg(iptr->val.l >> 32, REG_ITMP1);
				i386_alu_membase_reg(I386_CMP, REG_SP, src->regoff * 8 + 4, REG_ITMP1);
...
/*  					d = m->returntype; */
					...
					i386_mov_membase_reg(REG_ITMP2, OFFSET(vftbl, table[0]) + sizeof(methodptr) * m->vftblindex, I386_EAX);
...
u1 *createcompilerstub (methodinfo *m)
{
	...
	s4 *mcodeptr = p;					/* TWISTI: get rid of those 2nd defines */
...
/*  	                                       in reg 0 is used as method pointer */
	i386_mov_membase_reg(REG_SP, 8, I386_EAX);      /* load pointer to the compiler       */
	i386_jmp_reg(I386_EAX);             /* jump to the compiler, return address
	                                       in reg 0 is used as method pointer */
	/* TWISTI */
/*  	s[1] = (u8) m;                      /* literals to be adressed            */
/*  	s[2] = (u8) asm_call_jit_compiler;  /* jump directly via PV from above    */
	s[1] = (u4) m;                      /* literals to be adressed            */
	s[2] = (u4) asm_call_jit_compiler;  /* jump directly via PV from above    */
