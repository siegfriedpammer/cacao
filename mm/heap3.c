...
inline
static
void* advance_address(void* addr)
{
	if (*(u8*)addr & 0x1)
		return addr + (*(u8)addr & ~0x3);
		
	if (((java_objectheader*)addr)->vftbl == class_array->vftbl)
		addr = (void**)((long)addr + (long)((java_arrayheader*)addr)->alignedsize);
	else
		addr = (void**)((long)addr + (long)((java_objectheader*)addr)->vftbl->class->alignedsize);
}
...
inline
static
int get_type(void* addr)
{
	if (bitmap_testbit(markbits, addr))
		return USED;
...
void gc_reclaim (void)
{
 ...
 state1:
	...
	FREE:
		allocator_free_prejoin(addr1, addr, size);
		addr = advance_address(addr);
		goto state2;

	USED:
		allocator_free(addr1, size);
		size = 0;
		addr = advance_address(addr);
		goto state0;
	}
 ...
 state3:
	...
	FREE:
		allocator_free_blockjoin(addr1, addr, size);
		addr = advance_address(addr);
		goto state2;

	USED:
		allocator_free_postjoin(addr1, addr, size);
		addr = advance_address(addr);
		goto state0;
	}
